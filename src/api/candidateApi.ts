...
export const getCandidates = () =>
  axios.get<Candidate[]>('/Candidates/get-candidates', {
    useApiPrefix: true,
    useVoterBase: true
  });

export const getCandidateById = (candidateId: string) =>
  axios.get<Candidate>(`/Candidates/candidates/${candidateId}`, {
    useApiPrefix: true,
    useVoterBase: true
  });
...
export const addCandidate = async (data: CandidateCreateUpdate) => {
  ...
  const response = await axios.post('/Candidates/add-candidates', formData, {
...
export const updateCandidate = async (data: CandidateCreateUpdate) => {
  ...
  return axios.put('/Candidates/update-candidate', formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
    useApiPrefix: true,
    useVoterBase: true,
    transformRequest: (d) => d
...
export const deleteCandidate = (id: string) =>
  axios.delete(`/Candidates/delete/${id}`, { useApiPrefix: true, useVoterBase: true });

export const generateVoterSlip = (voterId: string) =>
  axios.get(`/Candidates/generate-voter-slip/${voterId}`, {
    useApiPrefix: true,
    useVoterBase: true
  });

export const getVoterSlip = (voterId: string) =>
  axios.get(`/Candidates/get-voter-slip-text/${voterId}`, {
    useApiPrefix: true,
    useVoterBase: true
  });
