...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getIcon = (toastType: ToastType) => {
    ...
    switch (toastType) {
      ...
      case 'error':
        return <MaterialIcons name="error-outline" size={20} color={iconColor} />;
      case 'warning':
        return <MaterialIcons name="warning-amber" size={20} color={iconColor} />;
      case 'info':
      default:
      ...
      {visible && (
        ...
        >
          <View style={styles.toastContent}>
            {getIcon(type)}
            <Text style={[styles.toastText, { color: textColors[type] }]}>{message}</Text>
          </View>
        </Animated.View>
      )}
...
const createStyles = (
  ...
  StyleSheet.create({
    toast: {
      ...
      borderRadius: 12,
      maxWidth: platform.isWeb && !platform.isMobileWeb ? 400 : screenWidth * 0.9,
      elevation: 5,
