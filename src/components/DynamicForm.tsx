...
export default function DynamicForm({
  ...
  const validate = () => {
    ...
    fields.forEach((field) => {
      ...
      if (field.type === 'password' && mode === 'create' && typeof value === 'string') {
        if (value.length < 6) {
      ...
      if (
        ...
        errors[field.name] = `${field.label} can only contain letters and spaces`;
      }
    });
  ...
  const handleChange = (field: FieldConfig, value: string | boolean) => {
    ...
    if (type === 'number') {
      if (field.decimalPlaces) {
        const decimalRegex = new RegExp(`^\\d*\\.?\\d{0,${field.decimalPlaces}}$`);
